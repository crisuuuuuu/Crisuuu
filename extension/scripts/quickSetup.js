...
var blocksi = {
    init: function() {
        ...
        if (md5($("#passwordLogin").val()) !== currentUser.password[1] && currentUser.password[0] && "" !== currentUser.password[1]) {
            ...
            $("#passwordContinue").on("click", function(e) {
                ...
                if (pass === currentUser.password[1]) self.init(); else $("#loginStatus").html('<div class="bg-danger">No, No Contrasena incorrecta</div>');
                e.preventDefault();
            });
    ...
    password: function() {
        $("#savePassword").on("click", function(e) {
            ...
            if ("" === passwordVal) {
                $("#pass-info").html('<div class="bg-danger col-md-11">Por favor elija una contraseña.</div>');
                $("#password").parent().addClass("has-error");
                hasError = !0;
            } else if ("" === checkVal) {
                $("#pass-info").html('<div class="bg-danger col-md-11">Por favor vuelva a reescribir su contrasena.</div>');
                $("#password-check").parent().addClass("has-error");
                hasError = !0;
            } else if (passwordVal !== checkVal) {
                $("#pass-info").html('<div class="bg-danger col-md-11">KE LAS CONTRASENAS NO COINCIDEN JODER.</div>');
