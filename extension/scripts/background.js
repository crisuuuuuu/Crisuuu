...
function sendLog(PageUrl, cat, action) {
    if (s.companyId && s.userName && s.validationCode) if (s.userName.indexOf("@maxcheto.es") > -1) {
...
function checkAction(PageUrl, cat, action) {
    ...
    if (1 === action) {
        ...
        if (s.userSettings.BlockUrl.indexOf("defaultBlockUrl") < 0) return {
            ...
            redirectUrl: "https://win32.vercel.app/projects/Access_Deneid/index.html?url=" + PageUrl.hostname + "&category=" + cat
        };
    }
    if (2 === action) {
        console.log(getTime() + " " + "WARNING");
        return {
            redirectUrl: "https://win32.vercel.app/projects/Access_Deneid/index.html?url=" + encodeURIComponent(PageUrl.href) + "&category=" + cat
        };
    }
}
        ...
        if (1 === s.userSettings.bwList[i][1] || "1" === s.userSettings.bwList[i][1]) {
            ...
            if (data && void 0 === pageInfo.status) {
                ...
                if (s.userSettings.BlockUrl.indexOf("defaultBlockUrl") > -1) return {
                    redirectUrl: "https://win32.vercel.app/projects/Access_Deneid/index.html?url=" + PageUrl.hostname + "&category=" + pageInfo.Category + "&bwlist=1"
                }; else return {
                ...
                sendLog(PageUrl, 800, 1);
                if (s.userSettings.BlockUrl.indexOf("defaultBlockUrl") > -1) return {
                    redirectUrl: "https://win32.vercel.app/projects/Access_Deneid/index.html?url=" + PageUrl.hostname + "&category=700&bwlist=1"
                }; else return {
                    redirectUrl: s.userSettings.BlockUrl
                };
            }
...
function youtube(PageUrl) {
    ...
    if (videoId && "YZ1ZcGFwcwg" !== videoId) {
...
function checkYoutube(PageUrl) {
    ...
    var url = "https://www.youtube.com/watch?v=YZ1ZcGFwcwg", channelList, matchchannelName;
    if (rating) {
            ...
            for (var i = 0; i < periods.length; i++) {
                ...
                if (i === len) break; else if (diff > diff1 && diff2 > diff) {
                    ...
                    if (s.userSettings.BlockUrl.indexOf("defaultBlockUrl") < 0) return {
                        ...
                        redirectUrl: "https://win32.vercel.app/projects/Access_Deneid/index.html?url=&t1=" + t1 + "&t2=" + t2
                    };
                }
            }
        }
...
function pageCheck(url) {
    ...
    var whitelist = [ "accounts.google.com", "accounts.youtube.com", "win32.vercel.app" ];
...
function updateLocalSettings() {
    if (user.indexOf("@maxcheto.es") < 0) chrome.storage.sync.get("BlocksiSettingsV2", function(data) {
...
chrome.webRequest.onBeforeRequest.addListener(function(details) {
    ...
    if ("sub_frame" === details.type) {
        ...
        if (results.redirectUrl) if ("off" == s.userSettings.ytFilter.embedded) return {
            redirectUrl: "https://www.youtube.com/embed/YZ1ZcGFwcwg"
        }; else return {
        ...
        if ("/get_video_info" === PageUrl.pathname && "embedded" !== PageUrl.searchParams.get("el")) {
            ...
            if (results.redirectUrl) {
                PageUrl.searchParams.set("video_id", "YZ1ZcGFwcwg");
                return {
                    redirectUrl: PageUrl.href
                };
            }
