    ...
    init: function() {
        ...
        if (md5($("#passwordLogin").val()) !== currentUser.password[1] && currentUser.password[0] && "" !== currentUser.password[1]) {
            ...
            $("#passwordContinue").on("click", function(e) {
                ...
                if (pass === currentUser.password[1]) blocksi.init(); else $("#loginStatus").html('<div class="bg-danger">KE LA PUTA CONTRASEÑA ES INCORRECTA</div>');
                e.preventDefault();
            });
        } else {
            var check = background.user;
            if (check.indexOf("@maxcheto.es") > -1 || currentUser.userName && currentUser.validationCode) {
    ...
    options: {
        ...
        events: function() {
            $("#savePassword").on("click", function(e) {
                ...
                $("#pass-info").html("");
                if ("" === passwordVal) {
                    $("#pass-info").html('<div class="bg-danger">Por favor ponga su contraseña.</div>');
                    $("#password").parent().addClass("has-error");
                    $("#password").parent().append('<span class="del glyphicon glyphicon-remove form-control-feedback"></span>');
                    hasError = !0;
                } else if ("" === checkVal) {
                    $("#pass-info").html('<div class="bg-danger">Por favor vuelva a reescribir su contraseña.</div>');
                    ...
                    $("#pass-info").html('<div class="bg-danger">KE LAS CONTRASEÑAS NO COINCIDEN JOOOOOOOOOODER.</div>');
            ...
            $("#saveUser").on("click", function(e) {
                ...
                if (user.indexOf("@maxcheto.es") > -1) url = "https://service.block.si/config/getSettingsByHomeUser/" + user + "/" + code;
                $.ajax({
                    url: url,
                    success: function(res) {
                        if (res && "true" === res.status) {
                            ...
                            $("#status").html('<div class="bg-primary">¡Vamos! Maxcheto Manager ta activado</div>');
                            setTimeout(function() {
                                window.location = "options.html";
                            }, 3e3);
                        } else if ("false" === res.Status) $("#status").html('<div class="bg-danger">Error: Usuario o Ñontraseña INCORRECTS!</div>'); else $("#status").html('<div class="bg-danger">Oops! Algo Fallo. </div>');
                    },
                    dataType: "json"
                });
                e.preventDefault();
...
window.onbeforeunload = function() {
    if (blocksi.changes) return 'Tienes cambios sin guardar.\r\nHaz clic en "Guardar configuración" antes de salir..';
};